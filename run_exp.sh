#!/bin/bash
export PATH="$PATH:$OMNET/bin"

PATH_NAME="$OMNET/samples/spider_omnet/benchmarks/circulations/"
GRAPH_PATH="$OMNET/samples/spider_omnet/scripts/figures/"
...
mkdir -p ${PATH_NAME}
if [ $# -ne 3 ]; then
    echo "Usage: $0 <routing_scheme> <pathChoice> <schedulingAlgorithm>"
    exit 1
fi
